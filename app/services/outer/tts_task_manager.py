...
class TTSTaskManager:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.db_manager.get_session() as session:
            # Get status counts
            status_counts = {}
            status_results = session.query(Task.status, func.count(Task.id)).group_by(Task.status).all()
            ...
            avg_file_size = session.query(func.avg(Task.file_size)).filter(
                and_(Task.status == 'completed', Task.file_size.isnot(None))
            ).scalar()

            # Get duplicate text hashes
            duplicate_results = session.query(
                Task.text_hash, func.count(Task.id).label('count')
            ).group_by(Task.text_hash).having(func.count(Task.id) > 1).all()
